...
export class TestGenerator {
  ...
  async generateTests(sourcePath: string, options: TestOptions): Promise<void> {
    console.log(chalk.gray('🧪 Analyzing source code for test generation...'));

    const files = await this.getSourceFiles(sourcePath);
    const testFiles = await this.generateTestFiles(files, options);

    console.log(chalk.green(`✅ Generated ${testFiles.length} test files`));
  }
  ...
  private async generateTestFiles(sourceFiles: string[], options: TestOptions): Promise<TestFile[]> {
  ...
  private async generateTestForFile(filePath: string, options: TestOptions): Promise<TestFile | null> {
  ...
  private generateTestCode(content: string, filePath: string, options: TestOptions): string {
    const fileName = path.basename(filePath);
  ...
  private generateJestSetup(fileName: string, imports: string[], exports: string[]): string {
...
describe('${this.toCamelCase(fileName)}', () => {
  ...
  private generateMochaSetup(fileName: string, imports: string[], exports: string[]): string {
...
describe('${this.toCamelCase(fileName)}', () => {
  ...
  private generateVitestSetup(fileName: string, imports: string[], exports: string[]): string {
...
describe('${this.toCamelCase(fileName)}', () => {
  ...
  private generateClassTests(classes: ClassInfo[], options: TestOptions): string {
    let testCode = '';
  ...
  private generateFunctionTests(functions: FunctionInfo[], options: TestOptions): string {
    let testCode = '';
  ...
  private generateExportTests(exports: string[], options: TestOptions): string {
    let testCode = '';
  ...
  private extractMethodsFromContent(content: string): MethodInfo[] {
  ...
  private extractPropertiesFromContent(content: string): PropertyInfo[] {
  ...
  private getExpectedType(returnType: string): string {
  ...
  private estimateCoverage(content: string): number {
    ...
    if (functions > 0) baseCoverage += Math.min(20, functions * 5);
    if (classes > 0) baseCoverage += Math.min(15, classes * 7);
